		...
		{ 
				...
				if(isPermission){
					...
					if(content.moduleLevel==1){
						if(content.url=='system_manager_index.html'){
						...
						if(content.url=='index.html' ||content.url=='survey_index.html'){
							return true;
						}
					}else if(content.moduleLevel==2){
						if(content.iconClass == 'ico8'){
							if(userInfoJson.degree == 0 || userInfoJson.degree==-1){
						...
						}else if(content.iconClass == 'ico1'){
							var html = '<li><a href="'+content.url+'" class="'+content.iconClass+'">'+content.moduleName+'</a></li><div name="secondMenu_'+content.id+'" id="secondMenu"><ul></ul></div>'
							$(".leftMenu").children("ul:first").append(html);
						}else{
							var html = '<li><a href="'+content.url+'" class="'+content.iconClass+'">'+content.moduleName+'</a></li><div name="secondMenu_'+content.id+'" id="secondMenu"><ul></ul></div>'
...
var index = function(){
	var _this={
		init:function(){
		    ...
		    if(userInfoJson.userType==2){
		    	$(".leftMenu a[url='/html/account/modPerson.html']").attr("url","/html/account/modCorp.html");
		    }
		    if(!indexPermission){
